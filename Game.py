...
#     if sec_menu:   #-----///----- this code fragment was ment to cope with too big footer, but problem lied in too big text passed to input
#         sec_sep_num = int(len(sec_menu[0]) / length) + 1    #second menu separation count number(deal with this name)(number of lines after separation)
#         menu += sec_menu            #adding these 2 together for separation in one go
...
#     if sec_menu:   #-----///----- this code fragment was ment to cope with too big footer, but problem lied in too big text passed to input
#         menu, sec_menu = menu[0:-sec_sep_num], menu[-sec_sep_num:]        #separating separated sec_menu from menu
#         length -= sec_sep_num - 1                   #reducing available length by appriopriate ammount(that '- 1' counts in already reduced amount at the beginning)
...
#         self.dmg_type_2 = dmg_type_2                #these are defined by weapon.equip() and char.unequip weapon() and same for armor
        ...
        # somehow this V V V doesn't work (somewhere else error with deepcoy pops up)
...
accept_cmsn = cmsn_story_3   #accepted comission (decided not to use list; only one can be accepted at one time)
                    ...
                    #---------complete (remind of) a comission------------
                        ...
                        while True:
                            ...
                            if a == 1:
                                ...
                                if accept_cmsn.type == 'hunt' or accept_cmsn.type == 'assasination':
                                    ...
                                    if kill_counter[1]:
                                        ...
                                        for char in global_char:
                                            ...
                                            out = out[:-1]
                                        sec_menu.append(out)
                                        sec_menu.append('Progress of comission will be updated.')
                                    else:
                                    ...
                                    cmsn_kill_counter = kill_counter
                                    #-----------------------------
            ...
            if a == 3:
                with open(savename, 'w') as save:
                    ...
                    if cmsn_kill_counter[0]:
                        ...
                        save.write('ckin')         #comission kill index?
                    ...
                    del exp_save, v, t_gold, t_gold_t, x, char, t_lis
